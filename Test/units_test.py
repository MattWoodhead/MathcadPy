...
# First, create a new instance of the Mathcad class (window can be set hidden with visible=False)
mathcad_app = Mathcad(visible=True)

ws_list = [
    Path.cwd() / "test_units.mcdx",
    "./test_units.mcdx",
]

for ws_path in ws_list:
    # Open a worksheet in Mathcad and set it as the active worksheet
    test_ws = mathcad_app.open(ws_path)
